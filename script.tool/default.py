#   script.maintenancetool, KhmerBox Tool
...
addonPath = os.path.join(os.path.join(xbmc.translatePath('special://home'), 'addons'),'script.maintenancetool')
...
def mainMenu():
    ...
    addItem('Clear All', 'url', 4,os.path.join(mediaPath, "all.png"))
...
def clearCache():
    ...
    dialog = xbmcgui.Dialog()
    dialog.ok("KhmerBox Tool", "Done Clearing Cache files")
...
def deleteThumbnails():
    ...
    dialog.ok("Exiting..", "Click OK to Exit and rebuild thumbnail library")
        ...
        for root, dirs, files in os.walk(purgePath):
            ...
            if file_count > 0:            
                ...
                for d in dirs:
                    shutil.rmtree(os.path.join(root, d))
                dialog = xbmcgui.Dialog()
                dialog.ok("KhmerBox Tool", "Deleting Packages all done")
            else:
                dialog = xbmcgui.Dialog()
                dialog.ok("KhmerBox Tool", "No Packages to Purge")
